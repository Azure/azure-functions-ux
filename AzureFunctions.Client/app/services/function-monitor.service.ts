...
export class FunctionMonitorService {
    ...
    getInvocationsDataForSelctedFunction(selectedFunctionName: string) {
        var url = this._functionsService.getScmUrl() + "/azurejobs/api/functions/definitions/" + selectedFunctionName.toLowerCase() + "/invocations?limit=20";
    ...
    getAggregateErrorsAndInvocationsForSelectedFunction(selectedFunctionName: string) {
        ...
        return this._http.get(url, {
            ...
            .map<FunctionAggregates>(r => r.json().entries.find(x => x.functionName === selectedFunctionName.toLowerCase()));
    }
}