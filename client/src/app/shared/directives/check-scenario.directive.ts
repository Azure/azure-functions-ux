...
export class CheckScenarioDirective implements OnChanges {
  @Input('check-scenario')
  id: string;
  @Input('cs-input')
  input: ScenarioCheckInput;
  @Input('cs-enabledByDefault')
  enabledByDefault = false;
  @Input('cs-enabledClass')
  enabledClass = '';
  @Input('cs-disabledClass')
  disabledClass = 'hidden';

  private _idStream = new Subject<string>();

  constructor(scenarioCheckService: ScenarioService, private _elementRef: ElementRef) {
    this._idStream
      .debounceTime(100)
      .switchMap(id => {
