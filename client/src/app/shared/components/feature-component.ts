...
export abstract class FeatureComponent<T> extends ErrorableComponent implements OnDestroy {
  ...
  constructor(componentName: string, injector: Injector, busyComponentName?: BusyStateName) {
    ...
    const preCheckEvents = this.__inputEvents.takeUntil(this.ngUnsubscribe).do(input => {
      ...
      if (this.__busyManager) {
        ...
        this.__telemetryService.featureLoading(this.isParentComponent, this.featureName, this.componentName);

        this.__logService.verbose(LogCategories.featureComponent, `New input received, setting busy componentName: ${this.componentName}`);
      }
    });
      ...
      .subscribe(
        r => {
          if (!this.__busyClearedEarly && this.__busyManager) {
            this.__logService.verbose(LogCategories.featureComponent, `Clearing busy normally componentName: ${this.componentName}`);
  ...
  protected setBusy() {
    ...
    this.__logService.debug(LogCategories.featureComponent, `Setting busy componentName: ${this.componentName}`);
  ...
  protected clearBusyEarly() {
    ...
    this.__telemetryService.featureLoadingComplete(this.featureName, this.componentName);
    this.__logService.verbose(LogCategories.featureComponent, `Clearing busy early componentName: ${this.componentName}`);
  ...
  protected clearBusy() {
    ...
    this.__logService.debug(LogCategories.featureComponent, `Clearing busy componentName: ${this.componentName}`);
