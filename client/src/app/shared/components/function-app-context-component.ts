...
export abstract class FunctionAppContextComponent extends ErrorableComponent implements OnDestroy {
  ...
  protected viewInfoEvents: Observable<
    TreeViewInfo<any> & {
  ...
  constructor(
    ...
    this.viewInfoEvents = this.viewInfoSubject
      ...
      .do(() => setBusy && setBusy())
      .switchMap(viewInfo =>
        Observable.zip(functionAppService.getAppContext(viewInfo.siteDescriptor.getTrimmedResourceId()), Observable.of(viewInfo))
      )
      .switchMap(tuple => {
  ...
  setup(): Subscription {
    return this.viewInfoEvents.takeUntil(this.ngUnsubscribe).subscribe(() => {
