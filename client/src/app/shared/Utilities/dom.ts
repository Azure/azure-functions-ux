export class Dom {
  ...
  public static getTabbableControls(element: HTMLElement): NodeList {
    return element.querySelectorAll(
      'input:not([disabled]), select:not([disabled]), textarea:not([disabled]), button:not([disabled]), object:not([disabled]), a:not([disabled]), area:not([disabled]), .link, div.sortable, [tabindex="0"]'
    );
  }
