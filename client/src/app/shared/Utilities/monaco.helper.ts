...
export namespace MonacoHelper {
  export function onResize(container: ElementRef, editorContainer: ElementRef, editor: MonacoEditorDirective) {
    ...
    if (editor) {
      editor.setLayout(editorContainerWidth - 2, editorContainerHeight - 2);
    }
  }
  ...
  export function onResizeFunction(
    ...
    if (codeEditor) {
      codeEditor.setLayout(editorContainerWidth - 2, editorContainerHeight - 2);
    }
    ...
    if (testDataEditor) {
      ...
      setTimeout(() => {
        if (testDataEditor) {
          testDataEditor.setLayout(rightTab ? widthDataEditor : 0, isHttpFunction ? 230 : functionContainerHeight / 2);
        }
      }, 0);
    }
  }
