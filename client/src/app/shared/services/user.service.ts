...
export class UserService {
  ...
  constructor(
    ...
    if (this.inIFrame || this.inTab) {
      this._portalService
        .getStartupInfo()
        .mergeMap(info => {
          return Observable.zip(Observable.of(info), this._getLocalizedResources(info, null), (i, r) => ({ info: i, resources: r }));
        })
        .subscribe(r => {
          const info = r.info;
      ...
      Observable.zip(this._getSubscriptions(null), this._getLocalizedResources(this._startupInfo, null), (s, r) => ({
        subscriptions: s,
        resources: r,
      })).subscribe(r => {
  ...
  getTenants() {
    return this._http
      .get(Constants.serviceHost + 'api/tenants')
  ...
  getAndUpdateToken() {
    return this._http
      .get(Constants.serviceHost + 'api/token?plaintext=true')
      .catch(() => {
  ...
  getUser() {
    return this._http.get(Constants.serviceHost + 'api/token').map(r => <User>r.json());
  }
  ...
  private _setToken(token: string) {
    if (token !== this._startupInfo.token) {
      Observable.zip(this._getSubscriptions(token), this._getLocalizedResources(this._startupInfo, null), (s, r) => ({
        subs: s,
        resources: r,
      })).subscribe(r => {
        const info = {
  ...
  private _getSubscriptions(token: string) {
    ...
    return this._http.get(url, { headers: headers }).map(r => <Subscription[]>r.json().value);
  }
  ...
  private _getLocalizedResources(startupInfo: StartupInfo<void>, runtime: string): Observable<any> {
    ...
    if (input.runtime === 'default') {
      ...
      const getResources = this._http
        .get(`${Constants.cdnHost}api/resources?name=${lang}&runtime=${input.runtime}&cacheBreak=${window.appsvc.cacheBreakQuery}`, {
    ...
    return this._http
      .get(`${Constants.serviceHost}api/resources?name=${input.lang}&runtime=${input.runtime}`, {
        headers: LanguageServiceHelper.getApiControllerHeaders(),
      })
      .retryWhen(LanguageServiceHelper.retry)
      .do(r => {
