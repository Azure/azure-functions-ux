...
export class FunctionsNode extends BaseFunctionsProxiesNode implements MutableCollection, Disposable, CustomSelection, Collection {
  ...
  constructor(sideNav: SideNavComponent, context: FunctionAppContext, parentNode: TreeNode) {
    super(sideNav, context.site.id + '/functions', context, parentNode, context.site.id + '/functions/new/function');
  ...
  protected _updateTreeForStartedSite(title: string, newDashboardType: DashboardType) {
    ...
    if (!this.children || this.children.length === 0) {
      return this._siteService.getFunctions(this._context.site.id).map(fcs => {
