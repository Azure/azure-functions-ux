...
export class AppsListComponent implements OnDestroy {
  ...
  constructor(public translateService: TranslateService, public broadcastService: BroadcastService, public route: ActivatedRoute) {
    this.broadcastService
      .getEvents<AppNode[]>(BroadcastEvent.UpdateAppsList)
      ...
      .subscribe(children => {
        ...
        this.locationOptions = this.uniqueLocations(this.apps).map(location => ({
          displayLabel: this.translateService.instant(location),
          value: this.translateService.instant(location),
        }));
        this.resourceGroupOptions = this.uniqueResourceGroups(this.apps).map(resourceGroup => ({
  ...
  onLocationsSelect(locations: string[]) {
    ...
    if (this.selectedLocations.length === this.locationOptions.length) {
      // if all locations are selected display all locations
      this._updateLocDisplayText(this.allLocations);
    } else if (this.selectedLocations.length > 1) {
      // else if more than 1 locations are selected display the number of locations
      this._updateLocDisplayText(this.numberLocations.format(locations.length));
    } else {
      // else 1 location is selected and its name is displayed
