...
import {
  ...
  CheckBoxListInput,
  EventGridInput,
} from '../../shared/models/binding-input';
...
export class BindingV2Component extends FunctionAppContextComponent {
  @Input()
  canDelete = true;
  @Input()
  canSave = true;
  ...
  @Input()
  allBindings: UIFunctionBinding[];

  @Output()
  remove = new Subject<UIFunctionBinding>();
  @Output()
  update = new Subject<UIFunctionBinding>();
  @Output()
  validChange = new Subject<BindingV2Component>();
  @Output()
  hasInputsToShowEvent = new Subject<boolean>();
  @Output()
  go = new Subject<Action>();
  @Output()
  ...
  constructor(
    ...
    this._broadcastService
      .getEvents<FunctionAppContext>(BroadcastEvent.IntegrateChanged)
    ...
    return this.viewInfoEvents
      ...
      .subscribe(tuple => {
        if (tuple[0].isSuccessful && tuple[1].isSuccessful) {
          this._appSettings = tuple[0].result.properties;
          this.authSettings = tuple[1].result;
          this.filterWarnings();
          this._updateBinding();
        }
      });
  ...
  private _updateBinding() {
    ...
    this.isDirty = false;
    const that = this;
    this._functionAppService.getBindingConfig(this.context).subscribe(bindingsResult => {
      if (!bindingsResult.isSuccessful) {
        this.showComponentError({
          message: bindingsResult.error.message,
          errorId: bindingsResult.error.errorId,
      ...
      const bindingSchema: Binding = this._bindingManager.getBindingSchema(
        this.bindingValue.type,
        this.bindingValue.direction,
        bindings.bindings
      );
      this.model.inputs = [];
        ...
        bindingSchema.settings.forEach(setting => {
          ...
          switch (setting.value) {
            case SettingType.int:
              ...
              intInput.help =
                this.replaceVariables(setting.help, bindings.variables) || this.replaceVariables(setting.label, bindings.variables);
  ...
  private _getAccountNameAndKeyFromAppSetting(settingName: string): string[] {
    ...
    if (value) {
      ...
      for (let i = 0; i < partsArray.length; i++) {
        ...
        if (accountNameIndex > -1) accountName = part.substring(accountNameIndex + 12, part.length);
        if (accountKeyIndex > -1) accountKey = part.substring(accountKeyIndex + 11, part.length);
      }
      account.push(value);
