...
export class FunctionNewDetailComponent implements OnChanges {
  ...
  private _exclutionFileList = ['test.json', 'readme.md', 'metadata.json'];
  ...
  onLanguageChanged(language: string) {
    ...
    if (this.functionLanguage) {
      this.onTemplatePickUpComplete().subscribe(() => {});
    }
  }
  ...
  onTemplatePickUpComplete() {
    ...
    return Observable.zip(this._functionAppService.getTemplates(this.context), this._functionAppService.getBindingConfig(this.context)).do(
      tuple => {
        if (tuple[0].isSuccessful && tuple[1].isSuccessful) {
          const templates = tuple[0].result;
          const bindingConfig = tuple[1].result;
          this.currentTemplate = templates.find(
            t => t.metadata.language === this.functionLanguage && !!this.functionCard.ids.find(id => id === t.id)
          );
          ...
          this.templateWarning =
            experimentalCategory === undefined ? '' : this._translateService.instant(PortalResources.functionNew_experimentalTemplate);
