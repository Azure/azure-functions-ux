...
export class ContainerImageSourceQuickstartComponent extends FeatureComponent<ContainerImageSourceData> implements OnDestroy {
  ...
  constructor(private _containerSampleService: ContainerSamplesService, injector: Injector) {
  ...
  protected setup(inputEvents: Observable<ContainerImageSourceData>) {
    return inputEvents
      ...
      .switchMap(containerImageSourceInfo => {
        ...
        return this._containerSampleService.getQuickstartSamples(
          this.containerImageSourceInfo.os,
          this.containerImageSourceInfo.container.id
        );
      })
      .do(containerSamples => {
  ...
  private _isContainerSampleMatch(sample: ContainerSample, key: string): boolean {
    ...
    return sample.containerOS === containerOS && sample.containerType === containerType && sample.name === name;
  }
}
