...
export class DeploymentCredentialsComponent extends FeatureComponent<string> implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    const passwordValidator = RegexValidator.create(
      /^((?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[\W]).{8,})$/, //The specified password does not meet the minimum requirements. The password should be at least eight characters long and must contain letters, numbers, and symbols.
