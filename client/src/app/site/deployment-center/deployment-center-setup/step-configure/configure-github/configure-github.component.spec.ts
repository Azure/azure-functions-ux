...
import { MockLogService } from '../../../../../shared/test-mocks/log.service.mock'; import { WizardForm } from '../../wizard-logic/deployment-center-setup-models';
...
class MockCacheService {
    ...
    private _getReposByPage(uri: string) {
        ...
        if (this.maxPagesRepo > 1) {
            testHeaders.append('link', `<https://api.github.com/resource?page=${page}>; rel="next",<https://api.github.com/resource?page=${this.maxPagesRepo}>; rel="last"`);
        }
        return Observable.of({
            headers: testHeaders,
            json: () => {
    ...
    private _getBranchesByPage(uri: string) {
        ...
        if (this.maxPagesBranch > 1) {
            testHeaders.append('link', `<https://api.github.com/resource?page=${page}>; rel="next",<https://api.github.com/resource?page=${this.maxPagesBranch}>; rel="last"`);
        }
        return Observable.of({
            headers: testHeaders,
            json: () => {
