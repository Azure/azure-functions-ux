...
export class WindowsConsoleComponent extends FeatureComponent<TreeViewInfo<SiteData>> {
  ...
  protected setup(inputEvents: Observable<TreeViewInfo<SiteData>>) {
     ...
     return inputEvents
       ...
       .do(
         ...
         err => {
           this._logService.error(LogCategories.cicd, '/load-windows-console', err);
           this.clearBusyEarly();
         });
   }
   ...
   private _onOptionChange(){
     if(this.currentOption == OptionsTypes.CMD){
