...
describe('GoToStepDirective', () => {
  ...
  it('should create an instance', () => {
    expect(wizardTestFixture.debugElement.query(By.css('wizard-navigation-bar')).queryAll(By.directive(GoToStepDirective)).length).toBe(3);
    expect(
      wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 1"]')).queryAll(By.directive(GoToStepDirective)).length
    ).toBe(3);
    expect(
      wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 2"]')).queryAll(By.directive(GoToStepDirective)).length
    ).toBe(2);
    expect(
      wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 3"]')).queryAll(By.directive(GoToStepDirective)).length
    ).toBe(1);
  ...
  it('should move to step correctly', () => {
    const firstStepGoToButton = wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 1"] > button:nth-child(2)'))
      .nativeElement;
    const secondStepGoToButton = wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 2"] > button')).nativeElement;
  ...
  it('should jump over an optional step correctly', () => {
    const firstStepGoToButton = wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 1"] > button:nth-child(3)'))
      .nativeElement;
    const thirdStepGoToButton = wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 3"] > button')).nativeElement;
  ...
  it('should stay at current step correctly', () => {
    const firstStepGoToButton = wizardTestFixture.debugElement.query(By.css('wizard-step[title="Steptitle 1"] > button:nth-child(1)'))
      .nativeElement;
  ...
  it('should throw an error when using an invalid goToStep value', () => {
    const invalidGoToAttribute = wizardTestFixture.debugElement
      ...
      .queryAll(By.directive(GoToStepDirective))[1]
      .injector.get(GoToStepDirective) as GoToStepDirective;

    expect(() => invalidGoToAttribute.destinationStep).toThrow(
      new Error(`Input 'goToStep' is neither a WizardStep, StepOffset, number or string`)
    );
  });
  ...
  it('should return correct destination step for correct goToStep values', () => {
    const firstGoToAttribute = wizardTestFixture.debugElement
      ...
      .queryAll(By.directive(GoToStepDirective))[0]
      .injector.get(GoToStepDirective) as GoToStepDirective;
