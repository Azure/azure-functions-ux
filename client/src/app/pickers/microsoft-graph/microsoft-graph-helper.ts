...
export class MicrosoftGraphHelper {
  ...
  getADDAppRegistrationInfo(necessaryAADPermisisons: AADPermissions[], graphToken: string): Observable<AADRegistrationInfo> {
    ...
    return this.checkForExistingAAD(rootUri).flatMap(applicationInformation => {
    ...
    return this.checkForExistingAAD(rootUri).flatMap(applicationInformation => {
      ...
      if (existingApplication) {
        ...
        return this.sendRequest(
          rootUri,
          '/applications/' + existingApplication.objectId + '/requiredResourceAccess',
          'PATCH',
  ...
  private setAuthSettings(applicationId: string, jwt, clientSecret, replyUrls) {
    ...
    return this._functionAppService.createAuthSettings(this.context, authSettings).do(null, error => {
      if (this._aiService) {
  ...
  private checkForExistingAAD(rootUri: string): Observable<any> {
    return this._cacheService.postArm(`${this.context.site.id}/config/authsettings/list`, true).flatMap(r => {
  ...
  private createClientSecret(rootUri: string, objectId: string): Observable<any> {
    ...
    return this._functionAppService.createAuthSettings(this.context, authSettings).do(
      () => {
...
export function CompareResources(current, necessary) {
  ...
  if (existingMSGraph) {
    ...
    unionMSGraph.resourceAccess = necessaryMSGraph.resourceAccess.concat(
      existingMSGraph.resourceAccess.filter(item => {
        return (
          necessaryMSGraph.resourceAccess.findIndex(necessary => {
  ...
  if (existingAAD) {
    // Union two arrays by removing intersection from existing resources then concatenating remaining
    unionAAD.resourceAccess = necessaryAAD.resourceAccess.concat(
      existingAAD.resourceAccess.filter(item => {
        return (
          necessaryAAD.resourceAccess.findIndex(necessary => {
            return necessary.type === item.type && necessary.id === item.id;
          }) < 0
        );
      })
