...
export class TryLandingComponent extends ErrorableComponent implements OnInit, OnDestroy {
    ...
    this._userService
      ...
      .subscribe(templates => {
        ...
        if (this._globalStateService.TryAppServiceToken) {
          ...
          if (selectedTemplate) {
            this.setBusyState();
            this._tryFunctionsService
              .createTrialResource(
                selectedTemplate,
                this._tryFunctionsService.selectedProvider,
                this._tryFunctionsService.selectedFunctionName
              )
              .subscribe(
                resource => {
                  this.clearBusyState();
                ...
                error => {
                  if (error.status === 400) {
                    ...
                    this._tryFunctionsService.getTrialResource(this._tryFunctionsService.selectedProvider).subscribe(resource => {
                      this.createFunctioninResource(resource, selectedTemplate, this._tryFunctionsService.selectedFunctionName);
                    });
                  } else {
                    this.clearBusyState();
    ...
    this._tryFunctionsService.getTemplates().subscribe(templates => {
      ...
      if (provider === '') {
        ...
        try {
          ...
          // login
          // get trial account
          this._tryFunctionsService.createTrialResource(selectedTemplate, provider, functionName).subscribe(
            resource => {
              this.clearBusyState();
            ...
            (error: Response) => {
              if (error.status === 401 || error.status === 403) {
                ...
                this._tryFunctionsService.getTrialResource(provider).subscribe(resource => {
                  this.createFunctioninResource(resource, selectedTemplate, functionName);
                });
              } else {
  ...
  createFunctioninResource(resource: UIResource, selectedTemplate: FunctionTemplate, functionName: string) {
    ...
    this._functionAppService.createFunction(this.context, functionName, selectedTemplate.files, selectedTemplate.function).subscribe(
      res => {
        this.clearBusyState();
        if (res.isSuccessful) {
          this._aiService.trackEvent('new-function', { template: selectedTemplate.id, result: 'success', first: 'true' });
          this._broadcastService.broadcast(BroadcastEvent.FunctionAdded, res);
          const navId = this.context.site.id
            .slice(1, this.context.site.id.length)
            .toLowerCase()
