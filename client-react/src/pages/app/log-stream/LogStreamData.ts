import { LogEntry, timerInterval, newLine, LogLevel, LogRegex, maxLogEntries } from './LogStream.types';
...
export function startStreamingRequest(
  ...
  const promise = new Promise<string>(() => {
    const hostNameSslStates = store.getState().site.data.properties.hostNameSslStates;
    const scmHostName =
    ...
    const callBack = () => {
      if (isStreaming && logStreamIndex !== xhReq.responseText.length) {
        ...
        if (newLogStream !== '') {
          ...
          updateLogEntries(newLogs);
        }
        logStreamIndex = xhReq.responseText.length;
        updateLogStreamIndex(logStreamIndex);
        window.setTimeout(() => {
...
// function buildRequest(xhReq: XMLHttpRequest, updateXhReq, allTimeouts: number[], updateTimeouts, webServerLogs: boolean) {
//   const hostNameSslStates = store.getState().site.data.properties.hostNameSslStates;
//   const scmHostName =
//     hostNameSslStates && hostNameSslStates.length > 0 ? hostNameSslStates.find(h => !!h.name && h.name.includes('.scm.'))!.name : '';
//   const suffix = webServerLogs ? 'http' : '';
...
export function _processLogs(logStream: string, oldLogs: LogEntry[]): LogEntry[] {
