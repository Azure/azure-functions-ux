...
export function setupDropboxAuthentication(app: Application) {
  ...
  app.get('/auth/dropbox/authorize', (req, res) => {
    ...
    res.redirect(
      `https://dropbox.com/oauth2/authorize?client_id=${process.env.DROPBOX_CLIENT_ID}&redirect_uri=${
        process.env.DROPBOX_REDIRECT_URL
      }&response_type=code&state=${oauthHelper.hashStateGuid(stateKey).substr(0, 10)}`
    );
  });
  ...
  app.post('/auth/dropbox/storeToken', async (req, res) => {
    ...
    if (
      !req ||
      !req.session ||
      !req.session[constants.oauthApis.dropbox_state_key] ||
      oauthHelper.hashStateGuid(req.session[constants.oauthApis.dropbox_state_key]).substr(0, 10) !== state
    ) {
