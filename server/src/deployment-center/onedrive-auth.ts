...
export function setupOnedriveAuthentication(app: Application) {
    ...
    app.get('/auth/onedrive/authorize', (req, res) => {
        ...
        res.redirect(`https://login.live.com/oauth20_authorize.srf?client_id=${process.env.ONEDRIVE_CLIENT_ID}&scope=offline_access,onedrive.readonly&response_type=code&redirect_uri=${process.env
                .ONEDRIVE_REDIRECT_URL}&state=${oauthHelper.hashStateGuid(stateKey).substr(0, 10)}`)
    ...
    app.post('/auth/onedrive/storeToken', async (req, res) => {
        ...
        try {
            const r = await axios.post(
                ...
                `code=${code}&grant_type=authorization_code&redirect_uri=${process.env.ONEDRIVE_REDIRECT_URL}&client_id=${process.env.ONEDRIVE_CLIENT_ID}&client_secret=${process.env
