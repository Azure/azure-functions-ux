...
export function setupBitbucketAuthentication(app: Application) {
  ...
  app.get('/auth/bitbucket/authorize', (req, res) => {
    ...
    res.redirect(
      `${constants.oauthApis.bitbucketUri}/authorize?client_id=${process.env.BITBUCKET_CLIENT_ID}&redirect_uri=${
        process.env.BITBUCKET_REDIRECT_URL
      }&scope=account+repository+webhook&response_type=code&state=${oauthHelper.hashStateGuid(stateKey).substr(0, 10)}`
    );
  });
  ...
  app.post('/auth/bitbucket/storeToken', async (req, res) => {
    ...
    if (
      !req ||
      !req.session ||
      !req.session[constants.oauthApis.bitbucket_state_key] ||
      oauthHelper.hashStateGuid(req.session[constants.oauthApis.bitbucket_state_key]).substr(0, 10) !== state
    ) {
