...
export class FunctionRuntimeComponent implements OnDestroy {
  ...
  updateVersion(version?: string) {
    ...
    this._cacheService.postArm(`${this.site.id}/config/appsettings/list`, true)
      ...
      .mergeMap(r => {
        return this.functionApp.getFunctionHostStatus().map((hostStatus: HostStatus) => {
          debugger;
          if (hostStatus.version === this.exactExtensionVersion) {
            throw Error('Host version is not updated yet');
          }
          return hostStatus;
        })
        .retryWhen(error => {
